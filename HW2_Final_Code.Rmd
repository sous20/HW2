...
date: "GitHub Repository Link - https://github.com/sous20/HW2"
output: pdf_document
---
...
The fourth weather situation is not present on the bar plot because there were no occurrences of it during the 9 AM hour on any day in the data set. Looking at each bar plot, we can see that overall ridership levels at 9 AM are higher during working days. However, the more important trend to observe is that as the weather becomes more severe, less people use the bike share system regardless of whether it is a working day or not. We can simply conclude from this observation that weather/environmental factors play a large role in determining demand for rental bikes.
...
average_board = group_by(metro, day_of_week, hour_of_day, month) %>%
  
  summarise(average_boarding = mean(boarding))

ggplot(average_board) + geom_line(aes(x = hour_of_day, y = average_boarding, color = month)) + facet_wrap(~day_of_week) + scale_color_brewer(type = "qual") + labs(title = "Hour of the Day vs. Average Boardings by Month and Day of the Week", x = "Hour of the Day (6AM to 9PM)", y = "Average Boardings", color = "Month")

```
&nbsp;

The figure above is a faceted line graph that displays the average number of people who board a Capital Metro bus in the UT area from 6 AM to 10 PM for every day of the week and every month displayed on the legend (September, October, and November). The hour of peak boardings stays fairly similar across most days, as we can see that people usually get on the bus at around 3 to 5 PM across the entire week. Average boardings on Mondays in September look lower compared to other days and months most likely due to the fact that the first Monday of every September is labor day. A full Monday without usual commuter traffic to work is bound to lower the average of people who take the bus on these days. Similarly, average boardings on Wednesdays, Thursdays, and Fridays in November look lower than usual most likely because people are off from work for Thanksgiving.

\newpage

#### Part 2

```{r, fig.align = "center", fig.height = 7, fig.width = 9.5, message = FALSE}

ggplot(metro) + geom_point(aes(x = temperature, y = boarding, color = weekend)) + facet_wrap(~hour_of_day) + labs(title = "Temperature vs. Boardings by Hour of the Day and Weekend Status", x = "Temperature (F)", y = "Total Boardings", color = "Weekend Status")

```
&nbsp;

The scatter plot above plots the relationship between temperature measured in degrees Fahrenheit and total boardings by the hour of the day and whether it was a weekend or weekday. Looking at each portion of the scatter plot, we can see that boardings stay relatively constant regardless of temperature (the points are evenly distributed horizontally along each scatter plot).
...
top_weeks = group_by(bill, performer, song) %>%
...
colnames(top_weeks) = c("Performer", "Song", "Count")
  
  top_weeks %>%
    
    kbl(caption = "Top Ten Most Popular Songs Since 1958 by Length in Weeks on Billboard 100") %>%
...
The table above displays ten songs that remained on the Billboard Top 100 for the longest time in history. It includes the name of each song, the artist for each song, and how many weeks each song was on the chart for.
...
year_diversity = filter(bill, year > 1958 & year < 2021) %>%
  
  group_by(year) %>%
  
  summarise("Num_unique" = length(unique(song)))

ggplot(year_diversity) + geom_line(aes(x = year, y = Num_unique)) + labs(title = "Year vs. Number of Unique Songs on the Billboard Top 100", x = "Year", y = "Number of Unique Songs")

```

&nbsp;

The line graph shown above plots the relationship between the year in history and the number of unique songs that made it to the Billboard Top 100. The most interesting trend is the massive dip in song variance from about 1970 up until 2000, when it began to increase again.
...
ten_week_hits = group_by(bill, performer, song) %>%
  
  summarise("num_weeks" = length(song)) %>%
  
  filter(num_weeks >= 10) %>%
  
  group_by(performer) %>%
  
  summarise("num_songs" = length(song)) %>%
  
  filter(num_songs >= 30)

ggplot(ten_week_hits) + geom_col(aes(x = performer, y = num_songs), color = "black", fill = "slateblue1") + labs(title = "Number of Ten Week Hits for Artists with At Least 30 Hits", x = "Artist", y = "Number of Ten Week Hits") + coord_flip()

```

&nbsp;

The bar plot above displays the number of ten week hits for 19 artists who have at least 30 ten week hits throughout their career. A ten week hit refers to a song that stayed on the Billboard Hot 100 for at least ten weeks.
